...
function extractNameFromTemplate(value) {
    return value.slice(value.indexOf('Hello') + 7, value.length - 1);
}
...
function removeFirstOccurrences(str, value) {
    const valueFirstIndex = str.indexOf(value);
    return str.slice(0, valueFirstIndex).concat(str.slice(valueFirstIndex + value.length));
}
...
function extractEmails(str) {
    if (str.search(/^((?:(?:[a-zA-Z0-9_\-\.]+)@(?:(?:\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(?:(?:[a-zA-Z0-9\-]+\.)+))(?:[a-zA-Z]{2,4}|[0-9]{1,3})(?:\]?)(?:\s*;\s*|\s*$))*)$/) > -1) {
        return str.split(';');
    }
}
